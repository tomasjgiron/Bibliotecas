...
*/
int generarArchivoLlamados(char* fileName,LinkedList* listaLlamados)
{
    Llamado* pLl;
    int auxId;
    char auxFecha[1500];
    ...
    FILE* fp = fopen(fileName,"w+");
    if(fp != NULL && listaLlamados != NULL)
    {
        fprintf(fp,"id_llamada,fecha,numero_cliente,id_problema,solucionado\n");

        size = ll_len(listaLlamados);

        for(i=0;i<size;i++)
        {
            pLl = ll_get(listaLlamados,i);
